...
export default function AuditLogsPage() {
  ...
  return (
    ...
    <div className="flex flex-col w-full h-[calc(97vh-4rem)] bg-slate-50 overflow-hidden relative">
      ...
      {/* --- HEADER (Fixed & Blended) --- */}
      {/* Removed bg-white, border-b, and shadow to remove the "Box" look */}
      <div className="flex-none p-6 pb-2 z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          {/* Filters Bar - Kept contained for contrast */}
      ...
      {/* flex-1 min-h-0 ensures THIS section scrolls, not the page */}
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr key={log.id} className="hover:bg-slate-50/80 transition-colors group">
                  ...
                  <td className="p-4 text-slate-500 truncate max-w-[250px] font-mono text-xs">
                     {log.newValues?.reason ? <span className="text-slate-700 font-semibold">Reason: "{log.newValues.reason}"</span> : 
                      log.action === 'UPDATE' ? 'Modified fields...' : 
                      <span className="opacity-70">Resource ID: {log.resourceId}</span>}
                  </td>
