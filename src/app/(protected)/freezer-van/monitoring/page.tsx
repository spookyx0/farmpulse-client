    ...
    <div className="p-2 md:p-6 space-y-6 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={fetchFleetData} className="p-2.5 bg-white border border-slate-200 text-slate-400 hover:text-cyan-600 rounded-xl shadow-sm transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex justify-between group">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Fleet Sales (Today)</p>
          ...
          <div className="w-12 h-12 rounded-full bg-cyan-50 flex items-center justify-center text-cyan-600"><TrendingUp className="w-6 h-6" /></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Active Vans</p>
            <p className="text-3xl font-black text-emerald-600 mt-1">{summary?.activeVans || 0}</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600"><Truck className="w-6 h-6" /></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Total Stock Value</p>
            <p className="text-3xl font-black text-blue-600 mt-1">₱{summary?.totalInventoryValue.toLocaleString(undefined, { minimumFractionDigits: 2 }) || '0.00'}</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center text-blue-600"><DollarSign className="w-6 h-6" /></div>
        </div>
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex justify-between">
          <div>
            <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Low Stock Alerts</p>
            <p className="text-3xl font-black text-orange-600 mt-1">{summary?.lowStockAlerts || 0}</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-orange-50 flex items-center justify-center text-orange-600"><AlertTriangle className="w-6 h-6" /></div>
        </div>
      </div>
